...
- **Perpetuals** — isolated margin, leverage, volume-weighted entry, liquidation
...
- **Production hardening** — JWT auth, per-IP rate limiting, load shedding,
  Prometheus metrics, OpenAPI docs, security headers, structured logging,
  graceful restore-on-restart.
...
packages/
  ...
  market-data   Upbit + Hyperliquid REST/WS connectors (real data)
apps/
...
- **~17,700 lines of TypeScript** (≈1:1 source-to-test ratio).
- **400+ tests**: unit, fast-check **property** tests (fund conservation, book
  integrity, reference-matcher cross-check, solvency), real-PGlite integration,
  **live** tests against the real Upbit/Hyperliquid APIs, and Playwright e2e.
